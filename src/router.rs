...
use crate::{http, ws, Req, Res, CONFIG};
...
pub async fn handle(mut request: Req) -> Result<Res, Error> {
  ...
  match target {
    Some(target) => {
      ...
      if !proxy {
        if is_ws {
          return Ok(Response::new(Body::from("not allowed update to websocket")));
        }
      ...
      if is_ws {
        ...
        tokio::spawn(async move {
          if let Err(e) = ws::server(request, websocket, domain).await {
            eprintln!("[Error] Websocket connection: {}", e);
          }
        });
